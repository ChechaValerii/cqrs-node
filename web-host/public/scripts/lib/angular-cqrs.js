 ...
 */
  .provider('CQRS', function CQRS() {
    ...
    let queryParserFunction = function (responseData) {
      return responseData;
    };
    let eventParserFunction = function (responseData) {
      return responseData;
    };
    let commandIdExtractionFunction = function (event) {
    ...
    this.toUrlGETParameterString = function (parameters) {
      const buffer = [];
      angular.forEach(parameters, (paramValue, paramName) => {
    ...
    this.$get = function ($q, $rootScope, $http, ObjectId) {
      ...
      function invokeCommandCallbackAndResolvePromise(event) {
        const commandId = commandIdExtractionFunction(event);
        const callback = commandCallbacks[commandId];
        if (angular.isDefined(callback)) {
          callback(event);
          commandCallbacks[commandId] = undefined;
        }

        const deferred = commandDeferreds[commandId];
      ...
      function sendCommand(commandObject, callbackFunction) {
        const augmentedCommandObject = augmentCommandObject(commandObject);
      ...
      function onEvent(listener) {
        $rootScope.$on('CQRS:events', (angularEvent, data) => {
          const event = eventParserFunction(data);
      ...
      function onCommand(listener) {
        $rootScope.$on('CQRS:commands', (angularEvent, data) => {
          listener(data);
        });
      }
 ...
 *
 */
  .service('DenormalizationService', () => {
    const denormalizerFunctions = {};
    ...
    return {
      getDenormalizerFunctions,
      registerDenormalizerFunction,
    };
  });
 ...
 */
  .service('ObjectId', () => {
    ...
    if (localStorageSupported()) {
      const mongoMachineId = parseInt(window.localStorage.mongoMachineId);
 ...
 */
  .service('StoreService', ($rootScope, $q, $filter, $timeout, CQRS, DenormalizationService) => {
    ...
    function onEventHandler(evt) {
      ...
      const denormalizerFunctions = DenormalizationService.getDenormalizerFunctions(evt.name, evt.aggregateType);
      angular.forEach(denormalizerFunctions, (denormalizerFunction, viewModelName) => {
        const scopeCallback = scopeCallbacks[viewModelName];
        if (angular.isDefined(scopeCallback)) {
          ...
          scopeCallback.callbacks.forEach((callback) => {
    ...
    function handleQueryResponse(queryResult, viewModelName, callback, scopeId) {
      if (angular.isDefined(scopeCallbacks[viewModelName])) {
        scopeCallbacks[viewModelName].callbacks.push({ callbackFunction: callback, scopeId });
    ...
    function removeCallbacksForScope(scope) {
      const orphanedStoreItems = [];

      angular.forEach(scopeCallbacks, (callbackItem, viewName) => {
        const cleanedCallbacks = $filter('filter')(callbackItem.callbacks, (callback) => (callback.scopeId !== scope.$id));
      ...
      orphanedStoreItems.forEach((orphan) => {
    ...
    function get(viewModelName, parameters, callback, errorCallback, scopeId) {
      ...
      const queryPromise = CQRS.query(viewModelName, parameters);
     ...
     */
    const StoreObject = function (scopeId) {
      /**
       * @ngdoc function
